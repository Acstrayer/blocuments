...
export default function Bloc(props: BlocProps) {
  ...
  const dragEnd = (e: any) => {
    ...
    const snapLeft = Math.round(((rect?.left || 0) - (gridRect?.left || 0)) / scale) + 1;
    const snapTop = Math.round(((rect?.top || 0) - (gridRect?.top || 0)) / scale) + 1;
    const colsWide = Math.ceil((rect?.width || 0) / scale);
    const rowsHigh = Math.ceil((rect?.height || 0) / scale);
    setGridPos(snapTop.toString()+" / "+snapLeft.toString()+" / "+(snapTop+rowsHigh).toString()+" / "+(snapLeft+colsWide).toString());
    console.log(snapTop.toString()+" / "+snapLeft.toString()+" / "+(snapTop+rowsHigh).toString()+" / "+(snapLeft+colsWide).toString());
    setIsDragging(false);
  };

  useEffect(() => {
    nodeRef.current?.firstChild?.addEventListener('mousedown', dragStart);
    document.addEventListener('mouseup', dragEnd);
    return () => {
      nodeRef.current?.removeEventListener('mousedown', dragStart);
  ...
  return (
    <div className={isDragging? "bloc dragging":"bloc"} style={isDragging? { left: coords.left, top: coords.top, width:size.width, height:size.height}:{gridArea: gridPos}} ref={nodeRef}>
